...
package body System.BB.Time is
   ...
   --  interrupts disabled, to ensure no stale values will be written.
   --  Given that limitation, it is fine to do concurrent updates on SMP
   --  systems: no matter which update ultimately prevails, it can't be old.
   --  While, on SMP systems, the Period_Counter may not always be monotone,
   --  the time returned by Update_Clock and Clock is.
   ...
   begin
      ...
      --  Reset Pending_Alarm before computing the next alarm time,
      --  as other processors may set alarms concurrently, and these alarms
      --  would be ignored otherwise. The alarm lock must be held for this.
      ...
      Threads.Queues.Wakeup_Expired_Alarms;
   ...
   begin
      ...
      --  read of the hardware clock, we know the time of that read, because
      --  the software clock can never be more than one period behind.
   ...
   begin
      ...
      --  updates of Software_Clock, or we lose track of time.  Allow a
      --  1/8th period safety for early wakeup. The alarm CPU should never
      --  have alarm interrupts disabled for longer than this, or we may miss
      --  clock updates.
   ...
   begin
      ...
      if Alarm < Pending_Alarm then
         ...
         Board_Support.Cancel_And_Set_Alarm (Clock_Interval (Time_Difference));
         Pending_Alarm := Alarm;
      end if;
   ...
   function To_Duration (TS : Time_Span) return Duration is
      Min_Time_Span : constant Time_Span :=
                 ...
                 Long_Long_Integer (System.OS_Interface.Ticks_Per_Second)));
   ...
   function To_Integer (TS : Time_Span) return Integer is
      Min_Time_Span : constant Time_Span :=
                          Time_Span (Long_Long_Integer'First /
                            (Long_Long_Integer (1.0 / Duration'Small) /
                    Long_Long_Integer (System.OS_Interface.Ticks_Per_Second)));
   ...
   begin
      ...
      pragma Assert (System.OS_Interface.Ticks_Per_Second <= 10#1#E9);
